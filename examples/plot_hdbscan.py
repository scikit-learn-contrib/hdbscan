...
def make_var_density_blobs(
    ...
    blobs = [
        make_blobs(n_samples=samples_per_blob, centers=[c], cluster_std=cluster_std[i])[
            0
        ]
...
X, labels_true = make_var_density_blobs(
    n_samples=750, centers=centers, cluster_std=densities, random_state=0
)
...
print(
    "Adjusted Rand Index: %0.3f" % metrics.adjusted_rand_score(labels_true, hdb_labels)
)
print(
...
print(
    "Adjusted Rand Index: %0.3f" % metrics.adjusted_rand_score(labels_true, db_labels)
)
print(
    "Adjusted Mutual Information: %0.3f"
